...
public class King {
     ...
     public static long[] KingAllPossibleMoves={
    ...
    public static long KingPossibleMoves(int place,Game game)
    {
        ...
        if (isPlayerPiece)
        {
            ...
            /// castling part.
            if(!game.isKingUnderThreat(isPlayerPiece)){
                if(((game.AllOccupiedPlaces | game.getComputerPossibleMovesLong()) & (bit<<2 | bit<<1))==0 && game.PlayerRightCastling)
                    PossibleMoves|=bit<<2;
                if(((game.AllOccupiedPlaces | game.getComputerPossibleMovesLong()) & (bit>>>3 | bit>>>2 | bit>>>1))==0 && game.PlayerLeftCastling)
                    PossibleMoves|=bit>>>2;
            }
        ...
        else
        {
            ...
            if(!game.isKingUnderThreat(isPlayerPiece)){
                if(((game.AllOccupiedPlaces | game.getPlayerPossibleMovesLong()) & (bit<<2 | bit<<1))==0 && game.ComputerRightCastling)
                    PossibleMoves|=bit<<2;
                if(((game.AllOccupiedPlaces | game.getPlayerPossibleMovesLong()) & (bit>>>3 | bit>>>2 | bit>>>1))==0 && game.ComputerLeftCastling)
                    PossibleMoves|=bit>>>2;
            }
